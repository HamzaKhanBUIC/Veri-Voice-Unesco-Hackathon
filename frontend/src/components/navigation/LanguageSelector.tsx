...
export const LanguageSelector: React.FC<LanguageSelectorProps> = ({
  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-surface-elevated/95 border border-white/10 shadow-2xl backdrop-blur-xl z-50 py-2 animate-fade-up">
          <div className="px-4 py-2 border-b border-border-subtle mb-1 flex items-center justify-between">
            <span className="text-[10px] font-mono uppercase tracking-widest text-text-muted">
              Select Language
            </span>
            <span className="text-[10px] font-mono text-brand-teal-bright">4 Active</span>
          </div>
